...
INSERT INTO accounts (id, user_id, balance, created_at, updated_at)
SELECT n, n, 0, NOW(), NOW()
FROM (
         SELECT 1000 + units.u + tens.t*10 AS n
         FROM (
...
WHERE n BETWEEN 1000 AND 1199;



INSERT INTO accounts (id, user_id, balance, created_at, updated_at)
SELECT n, n, 1000000, NOW(), NOW()
FROM (
         SELECT 1300 + units.u + tens.t*10 AS n
         FROM (
                  SELECT 0 AS u UNION ALL SELECT 1 UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL SELECT 4
                  UNION ALL SELECT 5 UNION ALL SELECT 6 UNION ALL SELECT 7 UNION ALL SELECT 8 UNION ALL SELECT 9
              ) units
             ...
             SELECT 0 AS t UNION ALL SELECT 1 UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL SELECT 4
             UNION ALL SELECT 5 UNION ALL SELECT 6 UNION ALL SELECT 7 UNION ALL SELECT 8 UNION ALL SELECT 9
             UNION ALL SELECT 10 AS t UNION ALL SELECT 11 UNION ALL SELECT 12 UNION ALL SELECT 13 UNION ALL SELECT 14
             UNION ALL SELECT 15 UNION ALL SELECT 16 UNION ALL SELECT 17 UNION ALL SELECT 18 UNION ALL SELECT 19
         ) tens
     ) seq
WHERE n BETWEEN 1300 AND 1499;



-- 아이템 100개 (id: 3001~3100, 가격 랜덤, 재고 1000개씩)
INSERT INTO items (id, name, price, created_at, updated_at)
SELECT 3000 + n, CONCAT('테스트 상품 ', n), (5000 + (n * 100) MOD 20000), NOW(), NOW()
FROM (
